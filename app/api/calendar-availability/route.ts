  ...
  try {
    ...
    if (!agentId) {
      return NextResponse.json(
        { success: false, error: 'agentId is required', slots: [] },
        { status: 400 }
      );
    }

    console.log(`[calendar-availability] Checking for agent ${agentId}, date: ${date}`);
    ...
    if (integrationError || !integration) {
      console.error('[calendar-availability] No active calendar integration:', integrationError);
      return NextResponse.json(
        {
          success: false,
    ...
    console.log(`[calendar-availability] Found ${integration.integration_type} integration`);
    ...
    if (integration.integration_type === 'gohighlevel') {
      ...
      try {
        ...
        if (!response.ok) {
          console.error('[calendar-availability] GHL API error:', response.status);
          return NextResponse.json(
            {
              success: false,
        ...
        console.log(`[calendar-availability] Found ${slots.length} available slots`);
        ...
        console.error('[calendar-availability] Error fetching GHL slots:', error);
        return NextResponse.json(
          {
            success: false,
